...
// ═══════════════════════════════════════════════════════════════
// FIX v4.2: ensureToken() — cải thiện, luôn trả về valid token
// ═══════════════════════════════════════════════════════════════
async function ensureToken() {
  const tok = getToken();
  if (tok && tok.trim() !== "") return tok.trim();
  // Fallback: pre-seed lại
  localStorage.setItem(TK, DASHBOARD_SECRET);
  console.warn("[AUTH] ensureToken: re-applied pre-seed →", DASHBOARD_SECRET);
  return DASHBOARD_SECRET;
}

// ═══════════════════════════════════════════════════════════════
// FIX v4.2: safeFetch — cải thiện 401 retry + token refresh
// ═══════════════════════════════════════════════════════════════
async function ensureToken() {
  localStorage.setItem(TK, DASHBOARD_SECRET);
  console.warn("[AUTH] ensureToken: synced dashboard token ->", DASHBOARD_SECRET);
  return DASHBOARD_SECRET;
}
...
async function boot() {
  try {
    ...
    // BƯỚC 5: Bootstrap API — FIX v4.2: retry với DASHBOARD_SECRET
    ...
    if (data?.ok) {
      ...
      if (data.violations) {
        // FIX v5.1: normalize cam_id → cam, đảm bảo image_url đúng
        data.violations.forEach(v => {
          if (!v.cam && v.cam_id) v.cam = v.cam_id;
          VIOLS.push(normalizeViolation(v));
        });
      ...
      // Init traffic + context + devices from bootstrap (before Socket.IO connects)
      if (data.traffic) {
        DS.light     = data.traffic.light || DS.light;
        DS.countdown = data.traffic.countdown ?? DS.countdown;
        DS.camState  = data.traffic.camera || DS.camState;
        DS.phase     = DS.light === "RED" ? "ĐỎ" : DS.light === "YELLOW" ? "VÀNG" : "XANH";
        renderTraffic();
      }
      if (data.context) {
        DS.speed    = data.context.speed_kmh ?? DS.speed;
        DS.vehicles = data.context.vehicles_frame ?? DS.vehicles;
        DS.capture  = Math.round(parseFloat(data.context.capture_interval ?? 0.5) * 1000);
        updateContext();
      }
      if (data.devices) {
        LIVE_DEVICES = data.devices;
        renderDevices();
        try {
      ...
      isDemo = false;
      addLog("[SYSTEM] Server/API chưa sẵn sàng. Không nạp dữ liệu giả; chờ camera và backend thật.", "warn");
      filtered = [...VIOLS];
      DS.totalViol = VIOLS.length;
