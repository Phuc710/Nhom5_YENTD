...
static void apply_state(tl_state_t state)
{
    ...
    case TL_STATE_RED:
        ...
        ESP_LOGI(TAG, "Đèn đỏ (mode=%d)", (int)s_mode);
        break;
    case TL_STATE_YELLOW:
        gpio_write(TL_PIN_YELLOW, 1);
        ESP_LOGI(TAG, "Đèn vàng (mode=%d)", (int)s_mode);
        break;
    case TL_STATE_GREEN:
        gpio_write(TL_PIN_GREEN, 1);
        ESP_LOGI(TAG, "Đèn xanh (mode=%d)", (int)s_mode);
        break;
    }
...
void traffic_light_init(void)
{
    ...
    ESP_LOGI(
        TAG,
        "Đèn giao thông sẵn sàng | Đỏ=%lums Vàng=%lums Xanh=%lums | GPIO R=%d Y=%d G=%d BtnR=%d BtnG=%d",
...
void traffic_light_set_mode(tl_mode_t mode)
{
    ...
    switch (mode) {
    case TL_MODE_NORMAL:
        ESP_LOGI(TAG, "Chế độ: NORMAL");
        break;
    case TL_MODE_EMERGENCY_RED:
        apply_state(TL_STATE_RED);
        ESP_LOGI(TAG, "Chế độ: EMERGENCY_RED");
        break;
    case TL_MODE_EMERGENCY_GREEN:
        apply_state(TL_STATE_GREEN);
        ESP_LOGI(TAG, "Chế độ: EMERGENCY_GREEN");
        break;
    }
}
...
void traffic_light_set_timings(uint32_t red_ms, uint32_t yellow_ms, uint32_t green_ms)
{
    ...
    ESP_LOGI(TAG, "Thời gian: Đỏ=%lums Vàng=%lums Xanh=%lums",
             (unsigned long)s_red_ms,
             (unsigned long)s_yellow_ms,
             (unsigned long)s_green_ms);
}
...
{
    (void)pvParameter;
    ESP_LOGI(TAG, "Task đèn giao thông khởi động");
    ...
    ESP_LOGI(TAG, "Task đèn giao thông kết thúc");
    vTaskDelete(NULL);
}
