/*
 * health_task.c — Theo dõi sức khỏe thiết bị và gửi telemetry lên ThingsBoard
 *
 * Thu thập mỗi HEALTH_CHECK_INTERVAL_MS (5s):
 *   free_heap, min_free_heap, wifi_rssi, uptime, frame/upload stats
 *
 * Gửi lên ThingsBoard mỗi TELEMETRY_INTERVAL_MS (30s).
 *
 * Dữ liệu upload (upload_ok, latency_ms, last_http_code) được định nghĩa
 * và cập nhật bởi uploader_task (đúng chủ sở hữu).
 */
...
/*
 * Dữ liệu upload — ĐỊNH NGHĨA trong uploader_task.c (owner đúng),
 * extern ở đây để đọc vào telemetry.
 */
...
{
    (void)pvParameter;
    ESP_LOGI(TAG, "Health task khởi động");
    ...
    ESP_LOGI(TAG, "Health task kết thúc");
    vTaskDelete(NULL);
}
