/*
 * task_manager.c — Khởi tạo queue và start tất cả FreeRTOS task
 */
...
/* traffic_light_task declared in traffic_light.h */
...
/* ---- Trạng thái chia sẻ ---- */
volatile uint32_t g_capture_interval_ms = 1000;
volatile bool     g_save_img            = true;
volatile int      g_camera_id           = 1;
...
esp_err_t task_manager_init(const char *token)
{
    ...
    if (cam_err != ESP_OK) {
        ESP_LOGW(TAG, "Khởi tạo camera thất bại (0x%x) — dùng fake mode", cam_err);
        g_camera_ok = false;
    } else {
        g_camera_ok = true;
    ...
    /* Khởi tạo đèn giao thông GPIO */
    traffic_light_init();

    /* Sao chép token cho MQTT task */
    ...
    /* Traffic light task (priority=6, cùng uploader) */
...
{
    if (!g_latest_frame_mutex || !data || len == 0) return;
...
void task_manager_report_event(const char *key, const char *value)
{
    ...
    xQueueSend(g_telemetry_queue, &msg, 0); /* Non-blocking */
}
