...
class ImageService:
    ...
    def __init__(self) -> None:
        ...
        # ThreadPool for Parallel Uploads (Lẹ!)
    ...
    def process_violation_images(
        ...
        """
        [LẸ] Generate and upload 3 evidence images IN PARALLEL.
        """
        ...
        # Each task: (image_type, bytes, filename)
        tasks = []
        ...
        if orig_bytes:
            tasks.append(("original", orig_bytes, f"{ts}_{uid}_t{track_id}.webp"))

        # 2. VEHICLE
        if vehicle_bbox is not None:
            vis = draw_vehicle_evidence(frame, vehicle_bbox, plate_text, plate_bbox, confidence)
            x1, y1, x2, y2 = vehicle_bbox
            ...
            if veh_bytes:
                tasks.append(("vehicle", veh_bytes, f"{ts}_{uid}_t{track_id}.webp"))

        # 3. PLATE
        if plate_bbox is not None:
            plate_img = draw_plate_crop(frame, plate_bbox, plate_text, min_w=200)
            plate_bytes = _encode_webp(plate_img, min(95, self._quality + 10))
            if plate_bytes:
                tasks.append(("plate", plate_bytes, f"{ts}_{uid}_t{track_id}.webp"))

        # Parallel Upload (Lẹ!)
        results = {
            "original_url": None, "vehicle_url": None, "plate_url": None,
            "full_image_url": None, "cropped_vehicle_url": None, "cropped_plate_url": None,
        }
        
        if tasks:
            from concurrent.futures import as_completed
            futures = {self._executor.submit(self._store, data, fname, itype): itype 
                       for itype, data, fname in tasks}
            
            for future in as_completed(futures):
                itype = futures[future]
                try:
                    url = future.result()
                    results[f"{itype}_url"] = url
                    # Aliases for DB
                    if itype == "original": results["full_image_url"] = url
                    elif itype == "vehicle": results["cropped_vehicle_url"] = url
                    elif itype == "plate": results["cropped_plate_url"] = url
                except Exception as e:
                    logger.error(f"❌ [IMAGE] Parallel upload error ({itype}): {e}")

        elapsed_ms = (time.perf_counter() - t0) * 1000
        logger.info(f"⚡ [IMAGE] cam{camera_id} processed 3 images in {elapsed_ms:.1f}ms (Parallel Upload)")
        return results
