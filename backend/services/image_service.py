"""
services/image_service.py — Lưu ảnh gốc và ảnh biển số crop
"""
import os
import uuid
...
class ImageService:
    ...
    def _unique_name(self, prefix: str, camera_id: int) -> str:
        ...
        return f"{prefix}_cam{camera_id}_{ts}_{uid}.jpg"

    async def save_image(self, image_bytes: bytes, camera_id: int, image_type: str = "original") -> str:
        """Lưu bytes JPEG → disk. Trả về đường dẫn tuyệt đối."""
    ...
    async def save_plate_image(self, plate_image: np.ndarray, camera_id: int) -> Optional[str]:
        """Lưu ndarray biển số crop → disk. Trả về đường dẫn tuyệt đối."""
